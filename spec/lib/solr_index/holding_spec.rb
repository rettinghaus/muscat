...
RSpec.describe Admin::HoldingsController, type: :controller, solr: true do
  FactoryBot.create(:person)
  let(:user) { create :admin   }
  render_views
  before(:each) do
    Sunspot.remove_all!(Holding)
    sign_in user
  end

  describe "creating holding record" do
   it "should change the solr holdings index result" do
      initial_size = Holding.solr_search { with("lib_siglum_order", "D-B")  }.total
      FactoryBot.create(:edition)
      Sunspot.index[Holding]
      Sunspot.commit
      after_create_size = Holding.solr_search { with("lib_siglum_order", "D-B")  }.total
