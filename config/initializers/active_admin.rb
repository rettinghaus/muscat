  ...
  config.namespace :admin do |admin|
    admin.build_menu :default do |menu|
      ...
      menu.add  :label => proc {I18n.t(:menu_languages)}, id: 'lang_menu', :priority => 2 do |lang|
        ...
        lang.add :label => "ES", :url => proc { url_for(:locale => 'es') }, id: 'i18n-es', :priority => 5, :html_options   => {:style => 'float:left;'}
        lang.add :label => "PT", :url => proc { url_for(:locale => 'pt') }, id: 'i18n-pt', :priority => 6, :html_options   => {:style => 'float:left;'}
      end
